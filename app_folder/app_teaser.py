...
#Get Top 10 Licensee's by Well Counts, then look at last 60 days Fracs...
# df= df.loc[(df['Licensee'].isin(counts.index[0:9]))&((pd.to_datetime('today') - pd.to_datetime(df['Last Fracture Date'])).dt.days<60)]
# df_total = df_total.loc[(df_total['Licensee'].isin(counts.index[0:9]))&((pd.to_datetime('today') - pd.to_datetime(df_total['Last Fracture Date'])).dt.days<60)]

#Columns for lower tables 
cols_displayed_well = ['Well Licence Number', 'Licensee',
                    'UWI', 'Number of Stages',
                'Production Fluid Type', 'Start Date',
...
#                 'background':'rgba(0,0,220,100)',
    ...
    html.Div([#SECOND ROW
        html.Div([
            dt.DataTable(
                        id="summary_table",
                        columns=[{"name": i, "id": i} for i in ['UWI', 'Well Licence Number', 'Licensee', 'Number of Stages',
                                                                'End Date','Total Proppant (tonnes)/Stage']],
                        data=df.loc[:,['UWI', 'Well Licence Number', 'Licensee', 'Number of Stages',
                        'End Date','Total Proppant (tonnes)/Stage']].to_dict("rows"), # initialise the rows
                        row_selectable="single",
                        selected_rows = [],
                        sorting=True,
                        # sorting_type="single",
                        derived_virtual_data=df.to_dict('rows'),
        ...
        html.Div(
             [dcc.Graph(id='teaser_graph_with_slider', config={'editable': True, 'modeBarButtonsToRemove': ['sendDataToCloud']})], 
             className='five columns')#No edit in cloud!
        ], className='row'    
    ),
    html.Div([#THIRD ROW
...
def update_teaser_figure(rows_total, selected_row_indices):
    ...
    filtered_df = df
    table_df = pd.DataFrame(rows_total) #Will use this for indexing selected rows into...
...
#***********************************SELECTED WELLS FROM TABLE DATA***********************************************************
    ...
    if selected_row_indices != None:
        ...
        traces.append(go.Scattergeo(
             #locationmode='CANADA',
                lon=filtered_df['Bottom Hole Longitude'][filtered_df['UWI'].isin(highlight_UWI_table)],
                lat=filtered_df['Bottom Hole Latitude'][filtered_df['UWI'].isin(highlight_UWI_table)],
                text=filtered_df['UWI'][filtered_df['UWI'].isin(highlight_UWI_table)] + ':'+ 'Well selected' ,
                customdata=filtered_df['UWI'][filtered_df['UWI'].isin(highlight_UWI_table)],
                mode='markers',
                hoverinfo='text',
                marker={
                    'color':'rgba(0,0,0,0)',
...
@app.callback(Output('teaser_selected_well_comp','data'),
              [Input('summary_table', 'derived_virtual_data'),
     Input('summary_table', 'derived_virtual_selected_rows')])
def teaser_update_selected_well_components(rows_total,selected_row_indices):
    ...
    if selected_row_indices != None:
        highlight_UWI_table= table_df['UWI'][selected_row_indices].tolist()
        return_df = df_total.loc[df_total['UWI'].isin(highlight_UWI_table),cols_displayed_comp]
...
@app.callback(Output('teaser_selected_well','data'),
              [Input('summary_table', 'derived_virtual_data'),
     Input('summary_table', 'derived_virtual_selected_rows')])
